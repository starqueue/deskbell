...
End-to-end flow, left to right: a login event is generated by the host;
deskbell reads it, normalises and de-duplicates it, rate-limits it, and
fans the resulting notification out to one or more transports; each
transport's server then broadcasts it to whichever subscribers you've
attached.

```mermaid
flowchart LR
    subgraph host["1. Host (kernel + system logs)"]
        direction TB
        L1["journald"]
        L2["/var/log/auth.log<br/>/var/log/secure"]
        L3["who(1) — polled"]
    end

    subgraph daemon["2. deskbell daemon (unprivileged)"]
        direction TB
        SRC["Sources<br/>journal, file, who<br/>(run in parallel)"]
        PRS["Parser<br/>SSH · PAM · login · who"]
        DED["Deduper<br/>60 s sliding window<br/>(user + origin + tty + method)"]
        RL["Rate limiter + digest<br/>6/min, burst 6<br/>(overflow → 60 s digest)"]
        FAN["Fan-out dispatcher<br/>(parallel, retried per transport,<br/>partial success counts as success)"]
        SRC --> PRS --> DED --> RL --> FAN
    end

    subgraph transports["3. Transports (any combination, all in parallel)"]
        direction TB
        NF1["ntfy server A<br/>HTTPS POST"]
        NF2["ntfy server B<br/>HTTPS POST"]
        SM["SMTP server<br/>STARTTLS or implicit TLS"]
    end

    subgraph subs["4. Subscribers (one topic → many simultaneously)"]
        direction TB
        PH["phone app<br/>(iOS / Android)"]
        BR["browser tab<br/>ntfy.sh/&lt;topic&gt;"]
        CL["curl pipe / ntfy CLI"]
        WH["Slack · Discord ·<br/>Matrix · SIEM<br/>(webhook bridges)"]
        EM["email inbox(es)"]
    end
